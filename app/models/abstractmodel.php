...
class AbstractModel
{
    ...
    public static function checkAuth($mail, $pass)
    {
        $sql = "SELECT * FROM " . static::$tableName . " WHERE mail = :mail and pass = :password";
        $stmt = DatabaseHandler::factory()->prepare($sql);
        ...
        if ($count == 1 && !empty($row)) {
            $_SESSION['session_id'] = $row['u_id'];
            $_SESSION['session_first_name'] = $row['firstName'];
            $_SESSION['session_user_name'] = $row['lastName'];
            $_SESSION['session_user_role'] = $row['Role'];
            // var_dump($_SESSION['session_id']);
            // var_dump($_SESSION['session_first_name']);
            return true;
